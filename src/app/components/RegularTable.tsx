...
export interface IRegularTableData {
  data: string;
  title?: string;
}
...
const RegularTable: React.FC<IRegularTableData> = ({ data, title }) => {
  ...
  return (
    <>
      {departmentNames.map((d, key) => {
        return (
          <>
            <Table caption={title || d} headers={headers} key={key}>
              <Day
